...
DWORD LaunchProcess(DWORD SessionId, const char** failed_operation)
{
    TCHAR buf[256];
    HANDLE hToken;
    swprintf_s(buf, __T("Starting , SessionId = %d"), SessionId);
    ...
    */
    ...
    ok = CreateProcessAsUser(hToken, (LPCWSTR)"c:\\windows\\system32\\notepad.exe", NULL, NULL, NULL, FALSE,
...
VOID WINAPI SvcMain(DWORD dwArgc, LPTSTR* lpszArgv)
{
    ...
    TCHAR buf[256];
    const TCHAR* title = __T("Zoho Corporation");
    ...
    while (!quit)
    {
        ...
        WTSSendMessage(WTS_CURRENT_SERVER_HANDLE, SessionId, (LPWSTR)title, _tcslen(title),
